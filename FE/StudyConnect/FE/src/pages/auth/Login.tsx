...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^0[0-9]{9}$/.test(phone)) {
      setError('Số điện thoại phải bao gồm đúng 10 chữ số bắt đầu bằng số 0')
      return
    }

    if (!/^[1-9]{6}$/.test(password)) {
      setError('Mật khẩu phải gồm 6 chữ số trong khoảng từ 1 đến 9')
      return
    }
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Đăng nhập thất bại. Vui lòng kiểm tra lại số điện thoại và mật khẩu.')
    } finally {
      setLoading(false)
    }
  }

  const placeholderStyle = "placeholder:text-gray-500 placeholder:opacity-40 dark:placeholder:text-gray-400 dark:placeholder:opacity-30"
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5 ml-1">
            <label className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider block">
              Mật khẩu <span className="text-[#FF6B00] text-[11px] font-normal normal-case ml-1">(6 số 1-9)</span>
            </label>
          </div>
          <input
            type="password"
            name="user_password_clean"
            id="user_password_clean"
            value={password}
